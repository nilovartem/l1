...
func worker(values <-chan int, wg *sync.WaitGroup, id int) {
	for value := range values {
		logrus.Infof("Value %v was received by worker %v\n", value, id)

		//когда небуферизованный канал закрыт,
		//то прога вылетает и часть с Exit не срабатывает
	}
